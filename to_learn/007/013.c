...
void draw_image(t_var *var, t_img *img, int x_start, int y_start, float scale) // change scale to float
{
	...
	while (y < y_start + height)
	{
		...
		while (x < x_end)
		{
			...
			while (modulo_y < 0)
				modulo_y += var->win_height; //* ((-y + var->win_height - 1) / var->win_height); // hDI HIYA CH7AL MENE MERA LOOP GHADI TDOR
											 // = ceil(-y / var->win_height) (3.4) -> 4
			while (modulo_x < 0)
				modulo_x += var->win_width; //* ((-x + var->win_width - 1) / var->win_width);*/
											//	printf("modulo_x: %d || modulo: %d \n", modulo_x, modulo_y);
...
int clicked_key(int keycode, t_var *var)
{
	...
	var->keycode = keycode;
	printf("is clicked = %d , message, code is %d\n", var->isclick, keycode);
	// printf("%d\n", keycode);
	// var->speed += 1;
	//  right 124
	//  down 125
...
int loop(t_var *var)
{
	...
	draw_image(var, &var->img, x, y, 3); // we draw the_made_image where we will put the image
...
/*int print_msg(t_var *var)
{
	int i;
	while(i < 20)
	{
	int j = mlx_key_hook(var->win, clicked_key, var); // use mlx_hook check documentation
	printf("return %d\n", j);
	i++;
	//}
	return (0);
