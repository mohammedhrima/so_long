...
int draw_rect(t_var *var, int x_start, int y_start, int x_end, int y_end, unsigned int color)
{
	...
	while (y < y_end)
	{
		...
		while (x < x_end)
		{
			var->draw_image[y * var->win_width + x] = color;
			x++;
		}
		y++;
	}
...
int loop(t_var *var)
{
	static int i = 0;
	i += 100;
	draw_rect(var, 0, 0, var->win_width, var->win_height, 0xffff); // we draw the black rectangle
	// draw_image(var, &var->img, i, 0, i, var->img.height / 2); //we draw image where we will put the image
...
int main(void)
{
	...
	var.win_width = 480;
	var.mlx = mlx_init();
	var.win = mlx_new_window(var.mlx, var.win_width, var.win_height, "Hello world!");

	// rectangle where we will put image
	var.draw_image_ptr = mlx_new_image(var.mlx, var.win_width, var.win_height);
	int bits_per_pixel, size_line, endian;
	var.draw_image = (unsigned int *)mlx_get_data_addr(var.draw_image_ptr, &bits_per_pixel, &size_line, &endian);

	var.img.ptr = mlx_xpm_file_to_image(var.mlx, "img/006.xpm", &var.img.width, &var.img.height);
	var.img.pixels = (unsigned int *)mlx_get_data_addr(var.img.ptr, &bits_per_pixel, &size_line, &endian);
