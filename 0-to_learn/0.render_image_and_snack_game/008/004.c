...
int draw_rect(t_var *var, int *x_start, int *y_start, int height, int width, int color)
{
	...
	while (w < width)
	{
		...
		while (h < height)
		{
			...
			var->draw_image[(*y_start + h) * WINDOW_WIDTH + (*x_start + w)] = color;
			h++;
		}
		w++;
	}
...
int clicked_key(int keycode, t_var *var)
{
	static int i = 0;
	static int j = 1;
	static int old_key_code = 0;

	var->keycode = keycode;
	if (keycode == RIGHT)
	{
		printf("%d. RIGHT\n", i);
		if (old_key_code == keycode)
			j++;
		else
			j = 1;
		var->snack_head[0] += j * LEN;
	}
	if (keycode == LEFT)
	{
		printf("%d. LEFT\n", i);
...
int main(void)
{
	...
	mlx_hook(var.win, 2, 0, clicked_key, &var);
	mlx_loop(var.mlx);
}