...
float time_to_sleep = TIME_TO_SLEEP;
// float old_time_to_sleep = TIME_TO_SLEEP;
static int last_clicked_key = 0;
...
int draw_rect(t_var *var, int *x_start, int *y_start, int height, int width, int color)
{
	...
	while ((int)w < width)
	{
		...
		while ((int)h < height)
		{
			...
			var->draw_image[(*y_start + (int)h) * WINDOW_WIDTH + (*x_start + (int)w)] = color;
			h++;
		}
		w++;
	}
...
int draw_snack(t_var *var)
{
	int x_zero = 0;
	int y_zero = 0;
	draw_rect(var, &x_zero, &y_zero, WINDOW_HEIGHT, WINDOW_WIDTH, BLACK); // draw black rect
	int j = 0;
	while (j < POINTS_LEN)
	{
...
{
	// static int last_key = RIGHT;
	static int i = 1;

	if (keycode == last_clicked_key)
	{
		time_to_sleep = TIME_TO_SLEEP -  (i * INCREASE_SPEED);
		i++;
	}
	else
	{
...
int draw(t_var *var)
{
	int x_zero = 0;
	int y_zero = 0;
	// static long old_time_to_sleep = TIME_TO_SLEEP;
	//  static float time_to_sleep = 40;
	draw_rect(var, &x_zero, &y_zero, WINDOW_HEIGHT, WINDOW_WIDTH, 0); // draw black rect
	// printf("hey 2\n");
	draw_moving_snack(var);
	// if (old_time_to_sleep == time_to_sleep)
	// 	time_to_sleep = TIME_TO_SLEEP;

	mssleep((long)time_to_sleep);
	//last_clicked_key = 0;
	//time_to_sleep = TIME_TO_SLEEP;
	// old_time_to_sleep = 0;
	// old_time_to_sleep = time_to_sleep;
	/*	if(time_to_sleep > 1)
			time_to_sleep -= INCREASE_SPEED;*/
