...
char *get_to_the_target(t_game game)
{
	...
	int i = 0;
	int j = 0;
	int fd = open("result.ber", O_RDWR | O_CREAT | O_TRUNC, 0777);
	while (i < WINDOW_HEIGHT)
	{
		j = 0;
		while (j < WINDOW_WIDTH)
		{
			write(fd, map_copy + i * WINDOW_WIDTH + j, 1);
			j++;
		}
		write(fd, "\n", 1);
		i++;
...
void put_one_pixel(t_var *var, int x, int y, int color)
{
	...
	// if (x >= SCALE * WINDOW_WIDTH)
	// 	x = x % (SCALE * WINDOW_WIDTH);
	// if (x < 0)
	// 	x = SCALE * WINDOW_WIDTH + x % (SCALE * WINDOW_WIDTH);
	// if (y >= SCALE * WINDOW_HEIGHT)
	// 	y = y % (SCALE * WINDOW_WIDTH);
	// if (y < 0)
	// 	y = SCALE * WINDOW_HEIGHT + y % (SCALE * WINDOW_HEIGHT);
...
int clicked_key(int keycode, t_var *var)
{
	...
	printf("(%d, %d), Player: %d, Width: %d, Height: %d \n", x, y, var->game.Player, WINDOW_WIDTH, WINDOW_HEIGHT);
	if (keycode == RIGHT && var->game.map[y * WINDOW_WIDTH + x + 1] != '1')
	{
		var->game.map[var->game.Player] = '0';
		var->game.Player = y * WINDOW_WIDTH + x + 1;
		var->game.map[var->game.Player] = 'P';
	}
	if (keycode == LEFT && var->game.map[y * WINDOW_WIDTH + x - 1] != '1')
	{
		var->game.map[var->game.Player] = '0';
		var->game.Player = y * WINDOW_WIDTH + x - 1;
		var->game.map[var->game.Player] = 'P';
	}
	if (keycode == UP && var->game.map[(y - 1) * WINDOW_WIDTH + x] != '1')
	{
		var->game.map[var->game.Player] = '0';
		var->game.Player = (y - 1) * WINDOW_WIDTH + x;
		var->game.map[var->game.Player] = 'P';
	}
	if (keycode == DOWN && var->game.map[(y + 1) * WINDOW_WIDTH + x] != '1')
	{
		var->game.map[var->game.Player] = '0';
		var->game.Player = (y + 1) * WINDOW_WIDTH + x;
		var->game.map[var->game.Player] = 'P';
...
int main(void)
{
	...
	var.mlx = mlx_init();
	var.win = mlx_new_window(var.mlx, SCALE * WINDOW_WIDTH, SCALE * WINDOW_HEIGHT, "Hello world!");
