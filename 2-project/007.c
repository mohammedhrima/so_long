...
int move(int Player, t_game game, int w, int h, char **was_here)
{
	...
	if (x < w - 1 && is_access((*was_here)[y * w + x + 1]) && n < 0)
		n = move(y * w + x + 1, game, w, h, was_here);
	if (x > 1 && is_access((*was_here)[y * w + x - 1]) && n < 0)
		n = move(y * w + x - 1, game, w, h, was_here);
	if (y < h - 1 && is_access((*was_here)[(y + 1) * w + x]) && n < 0)
		n = move((y + 1) * w + x, game, w, h, was_here);
	if (y > 1 && is_access((*was_here)[(y - 1) * w + x]) && n < 0)
		n = move((y - 1) * w + x, game, w, h, was_here);
	return (n);
}
