...
int Generate_map(int test, int max_collectible, int max_generating_time)
{
	printf("Generating map %d ...\n", test);
	int to_find[255];
	int to_find2[255];
	...
	// 	to_draw ="01C0EC1C0CC1PCE0C1C01CECCP01CC1CEPCC0101CPE01CPE";
	// else
	to_draw = "01C0EC1C0CC1PCE0C1C01CECCP01CC1CEPCC0101CPE01CPE"; // 01CE01PCE01CEP01CEP01CPE01CPE
	...
	while (j < WINDOW_HEIGHT - 1)
	{
		...
		while (i < WINDOW_WIDTH - 2)
		{
			...
			if ((to_draw[num] == 'P' || to_draw[num] == 'E' || to_draw[num] == 'C' || to_draw[num] == 'X') && to_find[to_draw[num]] && !stop)
			{
				...
				map[j * WINDOW_WIDTH + i] = '1';
			else if (to_draw[num] == 'X')
				map[j * WINDOW_WIDTH + i] = 'X';
			else
				map[j * WINDOW_WIDTH + i] = '0';
			i++;
			stop--;
...
int main(int argc, char **argv)
{
	...
	int max_generating_time = 5000;
	int max_collectible = 10;
	WINDOW_HEIGHT = 5;
	WINDOW_WIDTH = 10;

	int n = Generate_map(j, max_collectible, max_generating_time);
	while (n < 0 && i < 10000)
	{
		n = Generate_map(j, max_collectible, max_generating_time);
		i++;
		printf("sleeping... ");
		//	sleep(3);
	}
	if (n < 0)
		printf("failed , retry Please\n");
