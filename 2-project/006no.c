...
int m = 0;
int is_access(char c, int len)
{
	return ((c == '0' || c == 'C' || (c == 'E' && m == len)));
}
int move(int Player, t_game game, int w, int h, char **was_here)
{
	...
	if (!((*was_here)[game.Exit] == 'P' && m == game.Collectlen))
		n = -1;
	...
	if (n < 0 && x < w - 1 && is_access((*was_here)[y * w + x + 1], game.Collectlen))
		n = move(y * w + x + 1, game, w, h, was_here);
	if (n < 0 && x > 1 && is_access((*was_here)[y * w + x - 1], game.Collectlen))
		n = move(y * w + x - 1, game, w, h, was_here);
	if (n < 0 && y < h - 1 && is_access((*was_here)[(y + 1) * w + x], game.Collectlen))
		n = move((y + 1) * w + x, game, w, h, was_here);
	if (n < 0 && y > 1 && is_access((*was_here)[(y - 1) * w + x], game.Collectlen))
		n = move((y - 1) * w + x, game, w, h, was_here);
	return (n);
}
