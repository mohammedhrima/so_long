	...
	while (i < var->win_height)
	{
		...
		while (j < var->win_width)
		{
			...
			if (var->game.map[i * var->win_width + j] == 'X')
				draw_enemy(var, j * SCALE, i * SCALE, SCALE / 2);
...
int clicked_key(int keycode, t_var *var)
{
	...
	if (keycode == RIGHT && (ft_strchr("0CX", var->game.map[var->game.Player + 1]) || (var->game.Collectlen == 0 && var->game.map[var->game.Player + 1] == 'E')))
		var->game.Player++;
	if (keycode == LEFT && (ft_strchr("0CX", var->game.map[var->game.Player - 1]) || (var->game.Collectlen == 0 && var->game.map[var->game.Player - 1] == 'E')))
		var->game.Player--;
	if (keycode == UP && (ft_strchr("0CX", var->game.map[var->game.Player - var->win_width]) || (var->game.Collectlen == 0 && var->game.map[var->game.Player - var->win_width] == 'E')))
		var->game.Player -= var->win_width;
	if (keycode == DOWN && (ft_strchr("0CX", var->game.map[var->game.Player + var->win_width]) || (var->game.Collectlen == 0 && var->game.map[var->game.Player + var->win_width] == 'E')))
		var->game.Player += var->win_width;
