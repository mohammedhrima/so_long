...
int map_contains_allowed_chars(char *str, t_game *game, int w, int h)
{
	...
	while (str[i] && str[i] != '\n')
	{
		if (str[i] != '0' && str[i] != '1' && str[i] != 'C' && str[i] && str[i] != 'E' && str[i] != 'P' && str[i] != 'X')
			return (-2);
		if (str[i] == 'P')
		{
			if (game->Player)
...
void move_enemie(t_var *var)
{
	...
	while (i < var->game.Enemlen)
	{
		...
		if (last_move != 'r' && (var->game.map[y * WINDOW_WIDTH + x + 1] == 'P' || var->game.map[y * WINDOW_WIDTH + x + 1] == '0'))
		{
			var->game.Enemies[i] = y * WINDOW_WIDTH + x + 1;
			var->game.map[y * WINDOW_WIDTH + x + 1] = 'X';
			last_move = 'l';
		}
		else if (last_move != 'u' && (var->game.map[(y - 1) * WINDOW_WIDTH + x] == 'P' || var->game.map[(y - 1) * WINDOW_WIDTH + x] == '0'))
		{
			var->game.Enemies[i] = (y - 1) * WINDOW_WIDTH + x;
			var->game.map[(y - 1) * WINDOW_WIDTH + x] = 'X';
			last_move = 'd';
		}
		else if (last_move != 'l' && (var->game.map[y * WINDOW_WIDTH + x - 1] == 'P' || var->game.map[y * WINDOW_WIDTH + x - 1] == '0'))
		{
			var->game.Enemies[i] = y * WINDOW_WIDTH + x - 1;
			var->game.map[y * WINDOW_WIDTH + x - 1] = 'X';
			last_move = 'r';
		}
		else if (last_move != 'd' && (var->game.map[(y + 1) * WINDOW_WIDTH + x] == 'P' || var->game.map[(y + 1) * WINDOW_WIDTH + x] == '0'))
		{
			var->game.Enemies[i] = (y + 1) * WINDOW_WIDTH + x;
			var->game.map[(y + 1) * WINDOW_WIDTH + x] = 'X';
			last_move = 'u';
